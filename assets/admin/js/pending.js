(function($) {
    ...
    $(document).on("change","#pending_filter_lang",function(){
        table.ajax.url( $("#pending_filter_lang option:selected").data('href') ).load();
            var x = $(this).val();
    ...
    function pendingfilterLanguage(){
        table.ajax.url( $("#pending_filter_lang option:selected").data('href') ).load();
            var x = $("#pending_filter_lang").val();
            var url = mainurl+'admin/pending/category-filter/language/'+x;
            $.ajax({
                type        : 'GET',
                url         : url,
                contentType : false,
